...
#let font = (main: "Times New Roman", mono: "JetBrainsMono NF", cjk: "Noto Serif CJK SC")
...
#let assignment_class(size: 10.5pt, title, author, lesson_id, professor_name, semester, course_name, id, lang: "zh", region: "cn", body) = {
  ...
  set page(
    paper: "a4",
    header: locate(loc => if (counter(page).at(loc).first() == 1) { none } else {
      [
        #h(1fr)
        *#author* | *#title*
      ]
  ...
  let right_text = [*#professor_name*] + [#comma*#semester* ] + [| *截止时间：*#course_name]
